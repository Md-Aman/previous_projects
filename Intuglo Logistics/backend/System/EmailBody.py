...
class EmailBody(object):
        ...
        def customer_verification(to_email,temporaryPassword):
                customer_verification = '<html>' \
                                        '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                                        '<img src="cid:image2" align="middle">' \
                                        '<p align="left">Congratulations!</p>' \
                                        '<p align="left"> You have successfully subscribed to '+to_email+' Your temporary password is '+temporaryPassword+' </p>'\
                                        '<p align="left"> We would advise you to change to a new password of your own choosing immediately or at the earliest opportunity. This is to ensure that your access is kept secured at all times. Please click the button below to activate your account.</p>'\
                                        '<p align="left"> You are now ready to go global with Intuglo!</p>'\
        ...
        def forgotten_password(tokenID):
                forgotten_password = '<html>' \
                                ...
                                '<p align="left">Link to reset password: <a style="color: #fff;background-color: #FF7E2A;border-color: #FF7E2A;padding: 7px;font-size: 15px;width: 100%;text-decoration: none;" href="'+Tools.getClientURL()+'?token='+tokenID+'">Click Here</a></p>' \
                                '<p align="left">If you did not receive the email after 10 minutes, please check your spam folder or call our friendly Customer Service at (number). You can also click on the link to reset the password and try again.</p>'\
        ...
        def change_password(temporaryPassword):
                change_password = '<html>' \
                                '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                                '<img src="cid:image2" align="middle">' \
                                '<p align="left">Great job! You have successfully changed your password. Should you have any difficulty to login to your account, please email to customerservice@intuglo.com or call us at (….) </p>'\
                                '<p align="left">Thank you,</p>' \
                                '<p align="left">Password: ' + temporaryPassword + '</p>' \
                                '<p align="left">Intuglo Sales Team</p>'\
                                '<img align="middle" src="cid:image1" alt="testing">' \
                                '<p><a style="color: #fff;background-color: #FF7E2A;border-color: #FF7E2A;padding: 7px;font-size: 15px;width: 100%;text-decoration: none;" href="'+Tools.getClientURL()+'/">Login</a></p>' \
                                '</div></body>' \
                                '</html>'
        ...
        def upon_order(payment_id):
                ...
                cursor.execute("select p.transaction_no\
                                from ship_orders o\
                                left join payments p on o.payment_id = p.payment_id\
                                where payment_id ='"+payment_id+"'")
                quotation_info = cursor.fetchone()
                transaction_ref = quotation_info['transaction_no']
        ...
        def upon_setup_qt(quotation_id):
                ...
                cursor.execute("select q.quote_ref_no,q.container_box_count,q.arrival_date,q.departure_date,p1.port_name as PF,p2.port_name as PT\
                            ...
                            left join port p1 on q.port_id_from = p1.port_id\
                            left join port p2 on q.port_id_to = p2.port_id\
                ...
                upon_setup_qt = '<html>' \
                                ...
                                '<p align="left">Quotation ID: '+quotation_id+',</p>' \
                                '<p align="left">Quotation Reference: '+quotation_ref+ '</p>' \
                                '<p align="left">Containers For Sale: '+str(container_count)+ '</p>' \
                                '<p align="left">ETD: '+str(etd)+'</p>' \
                                '<p align="left">ETA: '+str(eta)+'</p>' \
                                '<p align="left">Departure Port: '+PortDep+'</p>' \
                                '<p align="left">Arrival Port: '+PortArrival+  '</p>' \
                                '<img align="middle" src="cid:image1" alt="testing">' \
                                '<p><a style="color: #fff;background-color: #FF7E2A;border-color: #FF7E2A;padding: 7px;font-size: 15px;width: 100%;text-decoration: none;" href="'+Tools.getClientURL()+'/">Click Here</a></p>' \
                                '<p align="left"> Thank you.</p>' \
        ...
        def custom_doc_download(quotation_id):
                ...
                custom_doc_download = '<html>' \
                                '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                                '<img src="cid:image2" align="middle">' \
                                '<p align="left"> Hello. We noticed that you have not completed uploading the required documents for Custom Clearance for '+orderID+'. Please upload them immediately.</p>' \
                                '<p align="left"> The last day for you to complete the upload is '+days_left_to_upload_doc+'. Failure to do so within the stated timeline may lead to an incomplete processing of your order. This will delay or lead to a cancellation of your shipment.</p>' \
        ...
        def authorize_payment_block(order_id):
                database_connection = get_db_connection()
                cursor = database_connection.cursor()
                cursor.execute("select o.order_id,v1.country_code as country_code_departure,v2.country_code as country_code_arrival, q.vessel \
                                from ship_orders o \
                                left join ship_quotations q on o.quotation_id = q.quotation_id \
                                left join ports v1 ON q.port_id_from = v1.port_id \
                                left join ports v2 ON q.port_id_to = v2.port_id \
                ...
                authorize_payment_block = '<html>' \
                                        '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                                        '<img src="cid:image2" align="middle">' \
                                        '<p align="left"> Thank you for your order '+orderID+' which is a shipment from '+country_from+' to '+country_to+'.</p>' \
                                        ...
                                        '<p><a style="color: #fff;background-color: #FF7E2A;border-color: #FF7E2A;padding: 7px;font-size: 15px;width: 100%;text-decoration: none;" href="'+str(Tools.getClientURL())+'">Click Here</a></p>' \
                                        '<p align="left"> Thank you.</p>'\
                                        '<p align="left"> Intuglo Operation Team</p>'\
                                        '</div></body>' \
                                        '</html>'
        ...
        def qt_closing_soon(quotation_id):
                qt_closing_soon = '<html>' \
                                '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                                '<img src="cid:image2" align="middle">' \
                                '<p align="left"> Quotation '+quotation_id+' closes in 5 days.</p>'\
        ...
        def qt_closed(quotation_id):
                ...
                quotation_info = cursor.fetchone()
                vesselName = quotation_info['vessel']
                etd = quotation_info['ETD']

                qt_closed = '<html>' \
                        '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                        '<img src="cid:image2" align="middle">' \
                        '<p align="left"> Quotation '+quotation_id+' is closed!</p>'\
                        '<p align="left"> All successful orders will be boarded on '+vesselName+' on '+etd+'.</p>' \
        ...
        def final_shipment_confirm(order_id):
                ...
                final_shipment_confirm = '<html>' \
                                        '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                                        '<img src="cid:image2" align="middle">' \
                                        '<p align="left"> We are pleased to inform you that your booking '+order_id+' is confirmed to be onboard '+vessel+'.</p>'\
        ...
        def final_payment_successful_deducted():
                final_payment_successful_deducted = '<html>' \
                                                '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">'\
                                                '<img src="cid:image2" align="middle">'\
                                                '<p align="left">Hello. The final payment of RMxxxx has been successfully deducted and the balance reverted to your account.</p>'\
                                                '<p align="left">Thank you for shipping with Intuglo. We look forward to your next shipment.</p>'\
                                                '<p align="left">Thank you.</p>'\
                                                '<p align="left">Intuglo Operation Team</p>'\
                                                '</div></body>'\
                                                '</html>'
        ...
        def payment_50():
                payment_50 = '<html>' \
                        '<body> <div align="center" style="height:400px;width:300px;padding:7px;margin-top:10px;">' \
                        '<img src="cid:image2" align="middle">' \
                        '<p align="left">Thank you for another successful shipment!</p>' \
                        '<p align="left"> Please note payment of 50% of the stated amount has been remitted/banked-in to your account as per the details on your profile. Please check your account.</p>'\
        ...
        def payment_40():
                payment_40 = '<html>' \
                        ...
                        '<img src="cid:image2" align="middle">' \
                        '<p align="left">Thank you for another successful shipment!</p>' \
                        '<p align="left"> Please note payment of 40% of the stated amount has been remitted/banked-in to your account as per the details on your profile. Please check your account.</p>'\
                        '<p align="left"> For your information, the bank takes 3 working days to clear the payment. In the event you did not receive the payment after 3 days, please call +603-78322188. Our account payable team shall provide the necessary clarification.</p>'\
                        '<p align="left">Thank you.</p>' \
                        '<p align="left">Intuglo Finance Team</p>' \
                        '<img align="middle" src="cid:image1" alt="testing">' \
