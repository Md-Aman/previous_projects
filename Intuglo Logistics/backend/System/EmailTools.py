...
class EmailTools(object):
    ...
    #         fp = open('C:\\Users\\HP EliteBook\\Documents\\IntugloLogistics\\backend\\System\\ship.JPG', 'rb')
    #         msgImage = MIMEImage(fp.read())
    #         fp.close()
    #         fp = open('C:\Users\\HP EliteBook\\Documents\\IntugloLogistics\\backend\\System\\logo.JPG', 'rb')
    #         logo = MIMEImage(fp.read())
    #         fp.close()
    ...
    #                        '<p style="text-align:center;color:Black;padding-top:15px;"><b>Thank you for your subscription</b></p>' \
    #                        '<p align="left">Hi '+userName+',</p>' \
    #                        '<p align="center">You have subscribe for Intuglo Logistics:</p>'\
    #                        '<img align="middle" src="cid:image1" alt="testing">'\
    #                        '<p align="center">Your almost ready to experience going global with intuglo.com. Please click on the button below to activate your account.</p>'\
    #                        '<a style="color: #fff;background-color: #FF7E2A;border-color: #FF7E2A;padding: 7px;font-size: 15px;width: 100%;text-decoration: none;" href="'+client_url+'/customer/activate?token='+tokenID+'">Activate your Account</a>'\
    #                        '</div></body>' \
    #                        '</html>'
    #         email_structure.attach(MIMEText(body_of_email, 'html'))
    #         server = smtplib.SMTP(server_address, server_port)
    ...
    #         email_info = cursor.fetchone()
    #         email_structure['Subject'] = quotation_info['email_subject']
    ...
    #         body_data = EmailTools.UponSetUpEmailNotify(quotation_id)
    #         email_structure.attach(MIMEText(body_data,'html'))
    #         server = smtplib.SMTP(server_address, server_port)
    #         server.starttls()
    #         server.login(from_address, from_address_password)
    ...
    # @staticmethod
    # def QTClosingSoonAndClosedEmailNotify(recipient,quotation_id,template_id):
    #     # TemplateID 9 & 10
    #     try:
    #         # Connecting the database
    #         database_connection = get_db_connection()
    ...
    #         from_address_password = row['config_value']
    #         email_structure = MIMEMultipart()
    #         cursor.execute("select email_subject from email_template where template_id ='"+template_id+"'")
    #         email_info = cursor.fetchone()
    #         email_structure['Subject'] = email_info['email_subject']
    ...
    #         body_of_email = EmailTools.QTClosingSoonAndClosedEmailNotify(quotation_id)
    #         email_structure.attach(MIMEText(body_of_email,'html'))
    #         server = smtplib.SMTP(server_address, server_port)
    #         server.starttls()
    #         server.login(from_address, from_address_password)
    ...
    def AuthorizePaymentBlockEmailNotify(order_id,recipient):
        try:
            ...
            row = cursor.fetchall()
            email_structure = MIMEMultipart()
            cursor.execute("select email_subject from email_template where template_id = 8")
            email_info = cursor.fetchone()
            email_structure['Subject'] = email_info['email_subject']
