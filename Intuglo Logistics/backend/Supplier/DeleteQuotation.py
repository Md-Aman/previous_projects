...
class DeleteQuotationResource__v1__(object):
        ...
        try :
            ...
            if (cursor.rowcount > 0):
                ...
                if (quotation_list[0]['quotation_status']== "DRAFT"):
                    ...
                    cursor.execute("update ship_quotations set quotation_status ='DELETED'\
                                 where quotation_id = '"+quotation_id+"'")
                    database_connection.commit()
                    resp.status = falcon.HTTP_200
                    message = {"Update": "Quotation deleted successfully"}
                    result_json = json.dumps(message)
                    resp.body = result_json
                else:
                    resp.status = falcon.HTTP_200
                    Err = {"Reason": "You are not allowed to delete this quotation."}
