...
export class CustomerCartComponent implements OnInit {
  ...
  constructor(
    ...
    this.subscription = this.service.getUpdateCartList().subscribe(details => {
      this.getCarList();
    });
  }
  ...
  getCarList() {
    // get list of orders in cart table
    this.amount = 0;
    this.service
      .getCartList(this.session.loginID, this.session.sessionID,this.session.cartID)
      .subscribe(
        response => {
          if (response.status == 200) {
            ...
            for (let i = 0; i < cart_details["cart_list"].length; i++) {
              ...
              // this.amount += cart_details["cart_list"][i]["transaction_fee"];
            }
            //console.log(this.cartItems);
            // this.tax_amount = (this.tax_rate / 100) * this.amount;
  ...
  downloadInvoice() {
    ...
    var creditBlockReceiptPDF = {
      ...
      content: [
...

        {
          ...
          columns: [
            ...
            {
              text: this.printDateFormatPipe.transform(this.invoiceDate),
              margin: [15, 0, 0, 1],
              color: "#424242",
              bold: true
            }
      ...
      footer:        
        {
          text: "This is an auto-generated invoice. No signature required.",
          color: "#424242",
          fontSize: 10,
          alignment: "center",
        }
    ...
    pdfMake.createPdf(creditBlockReceiptPDF).download("Booking Invoice");
  }

  // navigate to landing page if user wants to make another booking
  makeAnotherBooking() {
    this.sharedService
  ...
  getCartItemId(cart_item_id) {
    this.cartItemId = cart_item_id;
  }

  // delete order based on selected order id
  deleteOrders() {
    let cartItemId = {
      cartItemId: this.cartItemId
    };
    // sending cart item id to update order status to "deleted"
    this.service
      .updateCartStatusDeleted(
