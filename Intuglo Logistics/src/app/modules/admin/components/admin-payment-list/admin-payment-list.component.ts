import { Component, OnInit, Output, EventEmitter, Input, AfterViewInit } from '@angular/core';
import { AdminApiService } from './../../services/admin-api.service';
import { SharedService } from '../../../shared/services/shared.service';
import { Subscription } from 'rxjs/Subscription';
import { SessionStorage } from '../../../models/session-storage';
import * as $ from 'jquery';
import * as pdfMake from 'pdfmake/build/pdfmake';
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
import { PrintDateFormatPipe } from "../../../shared/pipes/print-date-format.pipe";
import { Constants } from './../../../util/constants';
import { ToastrService } from "ngx-toastr";

//all for month datepicker
import {FormControl} from '@angular/forms';
import {MomentDateAdapter} from '@angular/material-moment-adapter';
import {DateAdapter, MAT_DATE_FORMATS, MAT_DATE_LOCALE} from '@angular/material/core';
import {MatDatepicker} from '@angular/material/datepicker';
import * as _moment from 'moment';
import {default as _rollupMoment, Moment} from 'moment';
import { NG_VALUE_ACCESSOR, ControlValueAccessor } from '@angular/forms';
import { count } from 'rxjs/operators';
import { shiftInitState } from '@angular/core/src/view';


const moment = _rollupMoment || _moment;


export const MY_FORMATS = {
  parse: {
    dateInput: 'MM/YYYY',
  },
  display: {
    dateInput: 'MM/YYYY',
    monthYearLabel: 'MM YYYY',
    dateA11yLabel: 'LL',
    monthYearA11yLabel: 'MM YYYY',
  },
};

@Component({
  selector: 'admin-payment-list',
  templateUrl: './admin-payment-list.component.html',
  styleUrls: ['./admin-payment-list.component.css'],

    //for month picker
    providers: [
      {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},

      {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS}
    ]
})
export class AdminPaymentListComponent implements OnInit {

  //  session = new SessionStorage();
  countryList: any[];
  resourcesList: any[];
  vesselList: any[];
  selectedData;
  selectedCountry;
  selectedMonth;
  selectedCountryCode;
  selectedRoute;
  selectedVessel = null;
  selectedRouteName;
  selectedRouteId;
  selectedVesselName;
  selectedVesselId;
  displayCountryName: boolean = false;
  routeTabChanged: boolean = false;
  selectedIndex;
  month;
  year;
  amount;
  PaymentDetails;
  printablePaymentList:any =[];
  reportDate;
  paymentItems: any[];
  output: JSON;
  shipmentRoutesList: any[];
  message: boolean = false;
  displayTable: boolean = false;
  displayGenerateButton: boolean = false;

  filtershipmentRoutes:any = [];
  filterRoutes:any = [];
  filteredRoute:any = [];
  filterVessels:any = [];
  cards:any = []; 

  showCalendar: boolean = false;
  maxView = 'year';
  minuteStep = 5;
  minView = 'month';
  startView = 'month';
  views = ["minute", "hour", "day", "month", "year"];

  selectedDate: any;
  selDate: any;
  eventCounter: number = 0;
  customCal: boolean = false;
  eventCounter2: number = 0;


  constructor( 
    private printDateFormatPipe: PrintDateFormatPipe, 
    private service: AdminApiService, 
    private sharedService: SharedService,
    private toastr: ToastrService 
    ) { }

  session = new SessionStorage(); 

///////////////////----- Datepicker ------///////////////////
  date = new FormControl(moment());


  displayCalendar(event) {
    if (event) {
       this.eventCounter++;
       if (this.eventCounter % 2 === 0) {
         this.showCalendar = false;
         this.customCal = false;
       } else {
        this.customCal = true;
        this.showCalendar = true;
      } 
    }      
  }

onCustomDateChange(event){
  //there is a bootstrap bug and the function fires twice once page loads
        this.selectedMonth = moment(event.value, 'MM-YYYY').format('MM-YYYY');
        console.log('Formatted ' + this.selectedMonth);
        // if(this.selectedMonth){
        //   this.showCalendar = false;
        //   this.customCal = false;
        // }
        this.checkResources();
}
///////////////////----- Datepicker ends ------///////////////////


///////////////////----- Get Routes ------///////////////////




  ngOnInit() {
    this.reportDate = new Date();

         this.service.getCountryListForPayment(this.session.loginID,this.session.sessionID)
         .subscribe( getCountryList => {
         this.countryList = getCountryList;      
         console.log("Response country " + JSON.stringify(this.countryList));
         });
  }



  //////////----- Filtration -------///////
  
  checkResources(){
    try{
        if(this.selectedData !== undefined){
          this.selectedCountryCode = this.selectedData.country_code;
          this.selectedCountry = this.selectedData.country_name;
          console.log(this.selectedMonth);
          if(this.selectedMonth && this.selectedCountry !== undefined || null){
            this.getResources();
          } else {
            console.log('Select both, date and country');
            this.showInfo();
          } 
        }  
    } catch (error) {
      console.log('> Error is handled: ', error.name);
    } 
  } 

  getResources(){
    console.log("in resources " +this.selectedCountry + this.selectedMonth + this.selectedCountryCode);
               this.service.getAdminPaymentResources(this.session.loginID,this.session.sessionID, this.selectedMonth,
                 this.selectedCountry)
               .subscribe( getResources => {
                 this.resourcesList = getResources;
                 if(this.resourcesList !== null) {
                    this.shipmentRoutesList = this.resourcesList;                               
                    console.log("Response " + JSON.stringify(this.shipmentRoutesList));                
                    this.findCountryCode(this.selectedCountryCode);
                    this.displayCountryName = true;
                  } else {
                    console.log('no data found');
                    this.showNoDataFound();
                    this.displayCountryName = false;
                    this.selectedIndex = null;
                    this.filterRoutes = [];
                    this.filterVessels = [];
                 }
               });    
  }
  
   findCountryCode(countryCode){
    let filterShipmentRoutes2 = [];
    for(let shipmentRoute of this.shipmentRoutesList){ 
     if(shipmentRoute.country.includes(countryCode)){
       filterShipmentRoutes2.push(shipmentRoute.country);
       this.filtershipmentRoutes = filterShipmentRoutes2;
        //setting default first country code as selected
        this.onClickFindRoutes(this.filtershipmentRoutes[0]);   
       }
     }
   }


    onClickFindRoutes(route){ //MY-AU
      for(let routes of this.shipmentRoutesList){
        if(routes.country === route){
          this.filterRoutes = routes.route;
          this.selectedIndex = route;
        }
      }
  }

  onSelectedRoutes(){
    this.selectedRouteName = this.selectedRoute.portName;
    this.selectedRouteId = this.selectedRoute.route_id;
    for(let routes of this.filterRoutes){
      if(routes.route_id === this.selectedRouteId){
        this.filterVessels = routes.vessel;
        console.log(this.filterVessels);
      }
    }
  }

  ////////----- Filtration ends here ------//////

  getPaymentList() {
    // get list of payment

    if(this.selectedVessel !== null){
    this.selectedVesselName = this.selectedVessel.vessel_name;
    this.selectedVesselId = this.selectedVessel.vessel_id
    console.log(this.selectedVesselName);
    this.service.getAdminPaymentList(this.session.loginID, this.session.sessionID, this.selectedVesselId)
       .subscribe( getPayments => {
          this.PaymentDetails = getPayments;
         console.log(this.PaymentDetails);
          if(this.PaymentDetails != null) {
              this.displayTable = true;
              this.displayGenerateButton = true;
              console.log(this.PaymentDetails);

              for (let i = 0; i < this.PaymentDetails.length; i++) {
                         this.printablePaymentList.push({
                           no: i + 1,
                           booking_date: this.PaymentDetails[i]["booking_date"], 
                           closing_price_total: this.PaymentDetails[i]["closing_price_total"],
                           created_on: this.PaymentDetails[i]["created_on"],
                           customer_name: this.PaymentDetails[i]["customer_name"],
                           order_id:  this.PaymentDetails[i]["order_id"],
                           quotation_id: this.PaymentDetails[i]["quotation_id"],
                           supplier_name: this.PaymentDetails[i]["supplier_name"]
                         });
                       }
        
        }

         }
       );
      } else {
        console.log('select vessel first');
        this.showSelectVesselFirst();
      }
   }
  
  buildTableBody(data, columns) {
    var body = [];
    body.push([
        {
          text: "No",
          bold: true,
          margin: [0, 0, 0, 5],
          color: "#424242",
          fontSize: 11,
          fillColor: "#d1e2ff"
        },
      {
        text: "Booking Date",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      },
      {
        text: "Closing Price",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      },
      {
        text: "Created On",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      },
      {
        text: "Customer Name",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      },
      {
        text: "Order ID",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      },
      {
        text: "Quotation ID",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      },
      {
        text: "Supplier Name",
        bold: true,
        margin: [0, 0, 0, 5],
        color: "#424242",
        fontSize: 11,
        fillColor: "#d1e2ff"
      }
    ]);

    data.forEach(function(row) {
      var dataRow = [];
      columns.forEach(function(column) {
        dataRow.push(
          {
              text: row[column],
              color: "#424242",
              fontSize: 10
          }
        );
      });
      body.push(dataRow);
    });
    return body;
  }

  table(data, columns) {
    console.log(data)
    return {
      layout: 'lightHorizontalLines',
      table: {
        headerRows: 1,
        widths: ["3%","14%", "10%", "12%", '12%', '18%', '20%', '11%'],
        
        body: this.buildTableBody(data, columns)
      }
    };
  }


  onGenerateReport() {
    pdfMake.vfs = pdfFonts.pdfMake.vfs;

    var creditBlockReceiptPDF = {
      pageSize: "A4",

      content: [
        {
          canvas: [
            {
              type: "line",
              x1: 0,
              y1: 10,
              x2: 500 - -15,
              y2: 10,
              color: "#2674f2"
            }
          ],
          columnGap: 10
        },
        {
          columns: [
            {
              // star-sized columns fill the remaining space
              // if there's more than one star-column, available width is divided equally
              image:
                "data:image/png;base64,",
              fit: [150, 150],
              margin: [0, 12, 0, 0]
            },
          ],
          // optional space between columns
          columnGap: 10
        },     
        {
          canvas: [
            {
              type: "line",
              x1: 0,
              y1: 10,
              x2: 500 - -15,
              y2: 10,
              lineWidth: 10,
              color: "#2674f2"
            }
          ]
        },

        { text: 'Admin Payment Report', fontSize: 15, margin: [0, 30, 0, 30], alignment: 'center', color: "#424242", bold: true },

        {
          fontSize: 11, columns: [
               { 
                 width: 250, 
                 bold: true, text: 'Country: ' + this.selectedCountry,
                 margin: [0, 0, 0, 1],color: '#424242'
                           
               },
               { 
                width: 250, bold: true, 
                text: 'Month: ' + this.selectedMonth,
                margin: [130, 0, 0, 1],color: '#424242'
                           
               },
               
            ],

         },

         {
          fontSize: 11, columns: [
               { 
                 width: 250, 
                 bold: true, text: 'Route: '+ this.filtershipmentRoutes[0] + ', ' + this.selectedRouteName,
                 margin: [0, 0, 0, 1],color: '#424242'
                           
               },
               
            ],

         },

               {
                 fontSize: 11, columns: [
                   {  
                      text: 'Vessel: ' + this.selectedVesselName, 
                      margin: [0, 0, 30, 15],
                      bold: true, color: '#424242'},
                 ],
               },
   


        this.table(this.printablePaymentList, ["no","booking_date", "closing_price_total", "created_on", "customer_name", "order_id", "quotation_id", "supplier_name"]),

               
        // table starts here

        //content ends here
      ],
      footer: {
        columns: [
          { 
          text: 'Print Date: ' + this.printDateFormatPipe.transform(this.reportDate), 
          alignment: 'left',
          margin: [15, 0, 0, 0],
          color: "#424242",
          fontSize: 10,
          }, 
          {
            text: 'This is an auto-generated report.', 
            alignment: 'center',
            color: "#424242",
            fontSize: 10,
          },
          {
            text: 'Page 1' + ' of ' + '1', 
            alignment: 'right',
            margin: [0, 0, 25, 0],
            color: "#424242",
            fontSize: 10,
          }

        ]    
      }
    };

    pdfMake.createPdf(creditBlockReceiptPDF).download("Admin Payment Report");
  }

    ////////----- Toastr messages -------//////////
    showNoDataFound() {
      this.toastr.error("No Data Found", "Unsuccessful!", {
        closeButton: true,
        progressBar: true,
        progressAnimation: "increasing",
        positionClass: "toast-top-right"
      });
    }
  
    showSelectVesselFirst() {
      this.toastr.error("Select Vessel First", "Unsuccessful!", {
        closeButton: true,
        progressBar: true,
        progressAnimation: "increasing",
        positionClass: "toast-top-right"
      });
    }

    showInfo() {
      this.toastr.info("Select both, month and country", "Information!", {
        closeButton: true,
        progressBar: true,
        progressAnimation: "increasing",
        positionClass: "toast-top-right"
      });
    }
    
  }
